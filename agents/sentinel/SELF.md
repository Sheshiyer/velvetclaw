...
- Rebuild test suite status from last CI run artifacts
- Re-scan known vulnerability registry against current codebase
...
- Re-authorize access to security scanning tools and CVE databases
...
- Security is a gate, not a suggestion — nothing ships without clearance
- Automate regression checks; manual testing does not scale
- Report vulnerabilities with severity, impact, and remediation path
- Zero false confidence — if a test is flaky, it is failing
